...
Proof.
  intros n G.
  apply g_plus3. apply g_plus5. apply g_plus5. apply G. Qed.
(** [] *)
...
Proof.
  intros n m Hn Hm. induction Hn.
    ...
    simpl. apply g_plus5. apply IHHn.
  Qed.
...
Proof.
  intros x y z. rewrite plus_assoc. SearchAbout plus.
...
Proof.
  ...
  induction H.
    simpl. apply g_0.
    rewrite helper_g_times2. simpl. apply g_plus3. 
      rewrite <- helper_g_times2. simpl. apply g_plus3.
      apply IHgorgeous.
    rewrite helper_g_times2. simpl. apply g_plus5. 
      rewrite <- helper_g_times2. simpl. apply g_plus5.
      apply IHgorgeous.
  Qed.
...
Proof.
  intros n m Hnm Hn.
    induction Hn. apply Hnm.
    apply IHHn. inversion Hnm. apply H0. Qed.
...
Proof.
  ...
  apply ev_ev__ev with (n:=n+n) in Hnm. 
  apply Hnm.
  rewrite <- double_plus.
  apply double_even.
  rewrite <- plus_assoc.
...
Proof. intros X l H.
  induction H.
    reflexivity.
    reflexivity.
    simpl. SearchAbout snoc. rewrite rev_snoc. 
...
Lemma snoc_rev: forall X (x : X) l, rev (x :: l) = snoc (rev l) x.
Proof. intros. simpl. reflexivity. Qed.

Lemma pal_ends_eq: forall X (l l' : list X) x,
  pal l -> l = x :: l' -> l = snoc (rev l') x.
Proof. intros X l l' x P H.
  induction P.
    inversion H.
    inversion H. reflexivity.
    rewrite <- IHP. inversion H. simpl.
Abort.


Lemma pal_rev_converse: forall X (l : list X), l = rev l -> pal l.
Proof. intros X l H. induction l.
  apply pal_nil. 
  rewrite H. rewrite snoc_rev.
  induction l. simpl. apply pal_s. SearchAbout snoc. simpl. 
Abort.
...
Proof.
  intros m n o Hmn Hno. induction Hmn. apply Hno.
  apply IHHmn. apply le_Snm in Hno. apply Hno.
Qed.
...
Proof. 
  ...
  split. rewrite le_plus_l. apply H.
    rewrite le_plus_l. rewrite plus_comm in H. apply H.
Qed.
...
Proof.
  ...
  apply le_Snm in H. apply n_le_m__Sn_le_Sm. apply H.
Qed.
...
Proof.
  ...
  apply le_ble_nat.
  apply ble_nat_true in Hnm. apply ble_nat_true in Hmo.
  apply le_trans with (n:= m). apply Hnm. apply Hmo.
Qed.
...
Proof.
  intros n m eq H. induction H.
    rewrite <- ble_nat_refl in eq. inversion eq.
    apply le_S in H. apply le_ble_nat in H. 
...
Proof.
  intros Podd Peven n Hodd Heven. unfold combine_odd_even.
  destruct (oddb n).
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  unfold combine_odd_even. intros Podd Peven n Hc Hodd.
    rewrite Hodd in Hc. apply Hc.
Qed.
...
Theorem combine_odd_even_elim_even :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Peven n.
Proof.
  unfold combine_odd_even. intros Podd Peven n Hc Heven.
    rewrite Heven in Hc. apply Hc.
Qed.
