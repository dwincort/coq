...
Proof.
  intros b. induction b as [| |a1 a2|a1 a2|b1 IHb1|b1 IHb1 b2 IHb2];
    try reflexivity;
    try (simpl; rewrite optimize_0plus_sound;
         rewrite optimize_0plus_sound; reflexivity);
    try (simpl; rewrite IHb1; try rewrite IHb2; reflexivity).
Qed.
...
Proof.
  split.
  Case "->". intros H. 
    induction H as [ | |e1 e2 n1 n2 H1 H2|e1 e2 n1 n2 H1 H2
                     |e1 b1 H|e1 e2 b1 b2 H1 H2]; 
      try (apply aeval_iff_aevalR in H1; apply aeval_iff_aevalR in H2);
      subst; reflexivity.
  Case "<-".
    generalize dependent b.
      induction e as [ | |e1 e2|e1 e2|e1 H1| e1 H1 e2 H2];
      intros b H; subst; simpl; constructor; 
...
Proof.
  ...
  destruct (eq_id_dec x y).
    apply ex_falso_quodlibet; apply neq; apply e.
    reflexivity.
Qed.
...
Theorem update_eq : forall n x st,
  (update st x n) x = n.
Proof.
  intros n x st. unfold update. 
  destruct (eq_id_dec x x) as [eq|neq].
    reflexivity.
...
Proof.
  intros x2 x1 n st neq. unfold update. 
  destruct (eq_id_dec x2 x1) as [eq|neq'].
    apply ex_falso_quodlibet; apply neq; apply eq.
    reflexivity.
Qed.
...
Proof.
  intros n1 n2 x1 x2 st. unfold update.
...
Proof.
  intros n1 x1 x2 st st'. unfold update.
...
Proof.
  intros n1 n2 x1 x2 x3 st neq21. unfold update.
  destruct (eq_id_dec x1 x3);
    destruct (eq_id_dec x2 x3); subst; 
      try (apply ex_falso_quodlibet; apply neq21; reflexivity);
      try reflexivity.
Qed.
...
Theorem pup_to_2_ceval :
  pup_to_n / (update empty_state X 2) ||
    update (update (update (update (update empty_state
      X 2) Y 2) X 1) Y 3) X 0.
Proof.
  apply E_WhileLoop with (update (update (update empty_state X 2) Y 2) X 1).
    reflexivity. apply E_Seq with (update (update empty_state X 2) Y 2).
      apply E_Ass. reflexivity. apply E_Ass. reflexivity.
    apply E_WhileLoop with (update (update (update (update (update empty_state
                                X 2) Y 2) X 1) Y 3) X 0).
      reflexivity. apply E_Seq with (update (update (update (update empty_state
                                X 2) Y 2) X 1) Y 3).
        apply E_Ass. reflexivity. apply E_Ass. reflexivity.
...
Theorem XTimesYinZ_spec : forall st nx ny st',
  ...
  st' Z = nx * ny.
Proof.
  intros st nx ny st' Hx Hy Hev.
  inversion Hev. subst. simpl. apply update_eq.
Qed.
...
Proof.
  intros c. split. intros H.
    induction c; inversion H; try constructor.
      apply IHc1. apply andb_true_iff in H1.
      inversion H1. apply H0.
      apply IHc2. apply andb_true_iff in H1.
...
Proof.
  ...
  induction H as [|i a|c1 c2 Hc1 IHc1 Hc2 IHc2
                  |b c1 c2 Hc1 IHc1 Hc2 IHc2]; intros st.
    Case "SKIP".
      exists st. constructor.
    Case "Assignment".
      exists (update st i (aeval st a)). constructor. reflexivity.
    Case "Seq".
      destruct (IHc1 st) as [st' Hst]. 
      destruct (IHc2 st') as [st'' Hst'].
      exists st''. apply E_Seq with st'. assumption. assumption.
    Case "IFB".
      remember (beval st b) as b'. destruct b'.
      destruct (IHc1 st) as [st' Hst]. exists st'.
        apply E_IfTrue. symmetry. assumption. assumption.
      destruct (IHc2 st) as [st' Hst]. exists st'.
...
Theorem s_execute_app : forall st c1 c2 stack,
  s_execute st stack (c1 ++ c2) = s_execute st (s_execute st stack c1) c2.
Proof.
  intros st c1 c2. induction c1 as [| c c1'].
    simpl. reflexivity. 
    intros stack. destruct c; simpl; try apply IHc1';
      destruct stack; try apply IHc1'; destruct stack; apply IHc1'; apply IHc1'.
Qed.

Theorem s_compile_correct : forall (st : state) (e : aexp),
  s_execute st [] (s_compile e) = [ aeval st e ].
Proof.
  assert (forall (st : state) (e : aexp) stack,
    s_execute st stack (s_compile e) = aeval st e :: stack).
      intros st e. aexp_cases (induction e) Case; try reflexivity;
        intros stack; simpl; rewrite s_execute_app; rewrite IHe1;
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_SeqC : forall c1 c2 s st st' st'',
      c1 / st  || SContinue / st' ->
      c2 / st' || s / st'' ->
      (c1 ;; c2) / st || s / st''
...
Proof.
  ...
  inversion H2. inversion H5. reflexivity.
Qed.
...
Proof.
  intros b c st st' Hwhile Hbt. remember (WHILE b DO c END) as loop.
    induction Hwhile; inversion Heqloop.
      subst. rewrite Hbt in H. inversion H.
      subst. apply IHHwhile2. reflexivity. apply Hbt.
      subst. exists st. apply Hwhile.
Qed.
...
Theorem ceval_deterministic: forall (c:com) st st1 st2 s1 s2,
     ...
     st1 = st2 /\ s1 = s2.
Proof.
  intros c st st1 st2 s1 s2 H1. generalize dependent st2.
  generalize dependent s2. split.
    induction H1.
