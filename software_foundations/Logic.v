...
Proof.
  intros P Q R iffPQ iffQR. 
    ...
    split. intros. apply qr. apply pq. apply H.
    intros. apply qp. apply rq. apply H. Qed.
...
Proof.
  ...
  intros.
  destruct H as [B C].
  rewrite B. rewrite C. reflexivity. Qed.
...
Theorem contrapositive : forall P Q : Prop,
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q PQ NQ. unfold not.
  unfold not in NQ. intros.
  apply NQ in PQ. inversion PQ. apply H. Qed.
(** [] *)
...
Proof.
  unfold not. intros. apply H.
    right. intros. apply H. left. apply H0. Qed.
...
Proof. 
  intros. destruct (beq_nat n m) eqn:eq.
    unfold not in H. apply ex_falso_quodlibet. apply H.
      apply beq_nat_true in eq. apply eq.
    reflexivity. Qed.
(** [] *)
...
Proof.
  intros. unfold not. intros. destruct (beq_nat n m) eqn:eq. 
    inversion H.
    rewrite H0 in eq. rewrite <- beq_nat_refl in eq. inversion eq.
  Qed.
(** [] *)
