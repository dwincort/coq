...
Proof. 
  intros X P H. 
  unfold not. intros He. inversion He.
...
Proof.
  ...
  intros excmid X P H x.
  assert (P x \/ ~ P x) as excmid'. apply excmid.
  inversion excmid'. apply H0. destruct H. exists x. apply H0.
Qed.
...
Proof.
  intros X P Q. split. intros H. inversion H. inversion H0.
...
Proof. intros X test l. split.
  intros H. induction l.
    apply all_nil.
    apply all_cons. inversion H. destruct (test x).
      symmetry. apply H1.
      inversion H1.
      apply IHl. inversion H.
        destruct (test x). reflexivity. inversion H1.
...
Inductive in_order_merge {X : Type} : list X -> list X -> list X -> Prop :=
  ...
  | iom_l   : forall (x:X) (l r s : list X), 
      in_order_merge l r s -> in_order_merge (x::l) r (x::s)
  | iom_r   : forall (x:X) (l r s : list X), 
      in_order_merge l r s -> in_order_merge l (x::r) (x::s).

Theorem filter_spec :
  forall X (test : X -> bool) (l l1 l2 : list X),
    in_order_merge l1 l2 l ->
    all (fun x => test x = true) l1 ->
    all (fun x => test x = false) l2 ->
    filter test l = l1.
Proof. intros X test l l1 l2 iom all1T all2F. induction iom.
  reflexivity.
  inversion all1T. simpl. rewrite H1. apply f_equal. 
    apply IHiom. apply H2. apply all2F.
  inversion all2F. simpl. rewrite H1.
    apply IHiom. apply all1T. apply H2.
Qed.
...
Proof.
  intros X xs ys x. induction xs.
    intros H. right. simpl in H. apply H.
    intros H. inversion H.
      left. apply ai_here.
...
Proof.
  intros X xs ys x Hor. inversion Hor.
    induction H.
      simpl. apply ai_here.
...
Inductive no_repeats {X:Type} : list X -> Prop :=
  | no_reps_nil : no_repeats []
  | no_reps_cons : forall (x:X) (l:list X), 
      no_repeats l -> ~appears_in x l -> no_repeats (x::l).

(** Finally, state and prove one or more interesting theorems relating
    [disjoint], [no_repeats] and [++] (list append).  *)

Theorem disjoint_app_no_reps_help : 
  forall {X : Type} (x : X) (l1 l2 : list X), 
    appears_in x l1 -> appears_in x l2 -> ~ (no_repeats (l1 ++ l2)).
Proof.
  intros X x l1 l2 al1. induction al1.
    intros al2 contra. inversion contra. 
      apply H2. apply app_appears_in. right. apply al2.
    intros al2 contra. inversion contra. apply IHal1. apply al2. 
    apply H1.
Qed.

Theorem disjoint_app_no_reps : forall (X:Type) (xs ys : list X), 
     no_repeats (xs++ys) -> disjoint xs ys.
Proof.
  intros X xs ys noreps. unfold disjoint.
  intros x' contra. destruct contra as [axs ays].
  apply not_both_true_and_false with (P := no_repeats (xs ++ ys)). split.
  apply noreps.
    apply disjoint_app_no_reps_help with (x:=x').
      apply axs. apply ays.
Qed.
...
Inductive nostutter:  list nat -> Prop :=
  ...
  | ns_cons : forall a b xs, 
      nostutter (b::xs) -> ~(a=b) -> nostutter (a::b::xs).
...
Proof. 
  intros X l1 l2. induction l1. reflexivity. 
...
Proof.
  intros X x l ain. induction ain.
    exists []. exists l. reflexivity.
    inversion IHain. inversion H.
    exists (b::witness). exists witness0. simpl. apply f_equal. apply H0.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | reps_now   : forall (x:X) (l:list X), 
      appears_in x l -> repeats (x::l)
  | reps_later : forall (x:X) (l:list X), 
...
Proof.
  ...
  intros l2 excmid app_in_both lencomp.
    inversion lencomp.
  intros l2 excmid app_in_both lencomp.
Abort.
